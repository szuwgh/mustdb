...
#include <string.h>
#include <math.h>
/* ---- generic_copy_loop: MustDbVector → flat buffer copy ----
 *
...
void generic_copy_loop(MustDbVector* source, void* target, usize offset, usize element_count,
...
void copy_to_storage(MustDbVector* source, void* target, usize offset, usize element_count)
{
    generic_copy_loop(source, target, offset, element_count, false, NULL, NULL);
}

static inline bool check_dims(const MustDbVector* a, const MustDbVector* b)
{
    if (a->count != b->count)
    {
...
f32 vec_l2_squared_distance(const MustDbVector* a, const MustDbVector* b)
{
    if (!check_dims(a, b)) return NAN;
    return l2_squared_distance(a->count, (const f32*)a->data, (const f32*)b->data);
}

f32 vec_l2_distance(const MustDbVector* a, const MustDbVector* b)
{
    if (!check_dims(a, b)) return NAN;
...
f32 vec_inner_product(const MustDbVector* a, const MustDbVector* b)
{
    if (!check_dims(a, b)) return NAN;
...
f32 vec_cosine_distance(const MustDbVector* a, const MustDbVector* b)
{
    if (!check_dims(a, b)) return NAN;
...
f32 vec_l1_distance(const MustDbVector* a, const MustDbVector* b)
{
    if (!check_dims(a, b)) return NAN;
    return l1_distance(a->count, (const f32*)a->data, (const f32*)b->data);
}

f32 vec_compute_distance(const MustDbVector* a, const MustDbVector* b, DistanceType type)
{
    switch (type)
    {
        case L2_SQUARED:
