...
typedef struct
{
    usize size;  /* 对外可用的数据区大小，不包含 checksum/header */
    data_ptr_t internal_buf; /* 实际参与磁盘读写的对齐缓冲区起始地址，包含 header + data */
    usize internal_size; /* internal_buf 总大小，通常等于一个磁盘块大小 */
    data_ptr_t buffer;/* 用户读写的数据区起始地址，等于 internal_buf + FILE_BUFFER_HEADER_SIZE */
