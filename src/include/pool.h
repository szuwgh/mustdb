...
typedef enum BufferPoolPolicy
{
    BUFFER_POOL_POLICY_CLOCK_SWEEP = 1,
    BUFFER_POOL_POLICY_OLD_YOUNG = 2,
} BufferPoolPolicy;
...
struct Page
{
    ...
    bool valid; /* slot 是否已被占用；false 表示该 Page 为空闲槽，可直接装入新 Block */
    Page* hash_next;/* block_id 哈希冲突链；用于同一个 hash bucket 内链接多个 Page */
    Page* lru_prev; /* 未 pin 页面 LRU 双向链表前驱；pin_count==0 且 valid 时才会挂链 */
    Page* lru_next;   /* 未 pin 页面 LRU 双向链表后继；链表头是最近释放，链表尾是淘汰候选 */
    Page* dirty_prev; /* dirty list 前驱；dirty=true 时才会挂入 BufferPool.dirty_head 链 */
    Page* dirty_next; /* dirty list 后继；flush/recycle 写回后会从 dirty list 摘除 */
    Page* free_next; /* 空 frame 链；valid=false 时挂在 BufferPool.free_head 上 */
    ...
    atomic_bool io_in_progress; /* buffer slot 正在装载或写回，命中者必须等待 */
};

struct BufferPool
{
    Page* pages; /* 固定大小的 buffer slot 数组；每个 slot 缓存一个 Block/Page */
    usize num_pages; /* buffer slot 总数，也是 BufferPool 可同时缓存的最大页面数 */
    Page** hash_table; /* block_id -> BufferPage 的哈希桶数组，用于快速命中缓存页 */
    usize hash_size; /* hash_table 桶数量，保持为 2 的幂以便用位运算取桶 */
    Page* lru_head; /* 最近 unpin/add 的可淘汰页面 */
    Page* lru_tail; /* 最久未使用的可淘汰页面，优先作为 victim */
    Page* dirty_head; /* dirty 页面链表头；flush 从这里遍历，不再扫描所有 frame */
    Page* dirty_tail; /* dirty 页面链表尾；用于 O(1) 摘除/维护链表 */
    Page* free_head; /* 尚未使用的空 frame 链表头；pop 后即可装入新 Block */
