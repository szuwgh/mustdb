...
#define VECTOR_FOREACH(vec_ptr, entry_var)           \
    VectorIterator _iter_##entry_var;                \
    vector_iter_init(&_iter_##entry_var, (vec_ptr)); \
    void* entry_var;                                 \
    while (vector_iter_next(&_iter_##entry_var) &&   \
           ((entry_var = vector_iter_get(&_iter_##entry_var)), 1))
